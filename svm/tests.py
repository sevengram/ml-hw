...
class TestSVM(unittest.TestCase):
    ...
    def test_wide_slack(self):
        w = array([-.25, .25])
        b = -.25
        self.assertEqual(find_slack(self.insep_x, self.insep_y, w, b), {6, 4})

    def test_narrow_slack(self):
        w = array([0, 2])
        b = -5
        self.assertEqual(find_slack(self.insep_x, self.insep_y, w, b), {3, 5})

    def test_support(self):
        w = array([0.2, 0.8])
        b = -0.2
        result = find_support(self.sep_x, self.sep_y, w, b)
